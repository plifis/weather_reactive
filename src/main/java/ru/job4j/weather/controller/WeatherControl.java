...
public class WeatherControl {
    ...
    public Flux<Weather> getHottest() {
        return this.weathers.findMaxTemperature();
    }

    @GetMapping(value = "/cityGreatThen/{tem}")
    public Flux<Weather> getCurrentTemperature(@PathVariable int tem) {
        Flux<Weather> data = this.weathers.findAllCityWithTemperature(tem);
        Flux<Long> delay = Flux.interval(Duration.ofSeconds(3));
        return Flux.zip(data, delay).map(Tuple2::getT1);
    }
}
